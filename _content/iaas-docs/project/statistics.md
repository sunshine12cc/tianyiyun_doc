...
消费查询是用户级别的操作，在顶部导航 - 消费信息 - 消费统计可以找到入口。目前消费统计支持时间段选择、项目/资源类型/区域/账户的过滤，在查询的结果中，支持按项目、资源类型、区域、账户来分组查看。
...
查询时可以按项目、资源类型、区域、账号来过滤，也可以用搜索来精确指定一个具体的资源的消费情况。
...
消费预估是系统根据用户当前产生消费的资源租约，计算出的每小时、每天、每月、每年预估数额。并可以按项目、资源类型、账户、区域来分组给出预估数值。
