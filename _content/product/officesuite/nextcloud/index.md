...
![配置 Nextcloud 节点](create_nextcloud_nodes.png)
...
![创建 VPC](create_vpc.png)

点击加号创建一个私有网络：

![创建私有网络](create_vxnet.png)

等到 VPC 更新完毕后，就可以选择我们需要的私有网络了。

![第5步: 网络设置](network_config.png)

### 第6步: 服务环境参数设置

![第6步: 服务环境参数设置](env_cofnig.png)
...
![集群列表](cluster_lists.png)

点击集群 ID 可以查看该集群的详细信息：

![集群信息](cluster_apps_info.png)

集群的详细信息下方是对应节点的监控信息：

![监控信息](cluster_monitor.png)

### 配置参数

点击 `配置参数` 可以修改 `access_key_id`， `secret_access_key` 以及 `访问地址`。

![修改配置参数](change_env_config.png)

### 添加节点

点击 `新增节点` 可以增加对应的角色的节点，当前只支持增加 Nextcloud 节点。

![新增节点](add_nextcloud_node.png)
...
![控制集群](control_cluster.png)

点击 `重启` ，可以在集群故障时重启指定的角色：

![重启角色](restart_role.png)

点击 `扩容集群` ， 可以在集群性能不足时提高集群的配置：

![扩容集群](update_cluster_size.png)
...
![启用 Collabora Online](enable_collabora_online.png)

然后参考 [Collabora Office](../collabora-office/) 来部署 Collabora Office 服务。

然后在 NextCloud `设置 —— 在线协作` 中配置 Collabora Office 服务的地址并附上其对应的监听器所监听的端口。

![配置 Collabora Online](collabora_online_setting.png)
