...
![创建](../images/home_create_app.png)

进入创建页面之后，输入应用名称，例如：HBase 1.2.2。

![创建名字](../images/create_app.png)
...
我们提供了从简单到复杂的应用配置文件样例，详情请参看[通用模版规范定义](developer/docs/specifications/specifications.html)，也提供一些[案例](developer/docs/spec-samples)供参考。
...
- 安装自己的应用 <br>
- 安装agent <br>
  下载青云提供的 app agent [Linux 版本](https://pek3a.qingstor.com/appcenter/developer/packages/app-agent-linux-amd64.tar.gz), [Windows 版本](https://pek3a.qingstor.com/appcenter/developer/packages/app-agent-windows-386.zip)，解压后运行 ./install.sh (Windows 下双击 install.bat)。此 agent 中包含了自动配置文件程序 confd，该程序是在开源 [confd](https://github.com/kelseyhightower/confd/blob/master/docs/quick-start-guide.html) 的基础上修改了一些 bug 并且增加了一些算术功能，详情见 [QingCloud confd](https://github.com/yunify/confd/releases/tag/v0.13.0-alpha1)。

- 创建模版文件　<br>
  开发一些必须的模版文件，这些文件会监听青云metadata service的变化从而更新自己应用的配置文件。这些文件后缀名为toml和templ, 例如，ZooKeeper有两个配置文件 zoo.cfg和myid，每个配置文件需要一套相应的toml和tmpl模版对应。详情请见[配置ZooKeeper镜像](examples/images/zookeeper/image-guide.html)。
    ...
    toml文件里可加上权限控制 比如owner, mode等，详情请见[confd](https://github.com/kelseyhightower/confd/blob/master/docs/quick-start-guide.html)
    ...
    更多模版语法参见[confd templates](https://github.com/kelseyhightower/confd/blob/master/docs/templates.html)， 注意的是青云的confd在开源基础上增加了一些对算术的支持，如add,div,mul,sub,eq,ne,gt,ge,lt,le,mod

#### 制作Docker镜像

详细参看 [Docker 指南](app-version-mgmt/images/docker.html)
...
![my_apps](../images/my_apps.png)
![app_cluster_list](../images/app_cluster_list.png)
![app_cluster](../images/app_cluster.png)

### 提交审核

若当前版本各方面测试已通过，可以将此应用版本提交审核。但需要注意的是，提交审核之前请完善必要的应用信息，否则会影响审核结果。此情况下页面上也会有相应提示。

![before_submit_app](../images/before_submit_app.png)

提交之后，应用版本的状态将更新为“审核中”。

![app_submitted](../images/app_submitted.png)

审核工作将在10个工作日内完成。当审核被拒绝时页面上会显示详细的拒绝原因，请调整后重新提交审核。

![reject_app_version](../images/reject_app_version.png)
...
![release_app_version](../images/release_app_version.png)
...
![app_users](../images/app_users.png)

必要时，开发者可以点击用户 ID 查询该用户名下的集群实例列表，或做一些过滤查询。

![app_user](../images/app_user.png)
...
参考 [Metadata Service](metadata-service.html)

## 应用模版规范

参考 [应用开发模版规范](specifications/specifications.html)
